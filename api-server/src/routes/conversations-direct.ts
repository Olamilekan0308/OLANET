...
// Authentication is still performed with the user's session above. Database reads/writes
// below deliberately use the server service key when available so RLS cannot break a valid
// authenticated DM operation. The service key never leaves this server.
async function hasRelationship(a: string, b: string) {
  const r = await sb(`/rest/v1/friendships?or=(and(requester_id.eq.${encodeURIComponent(a)},addressee_id.eq.${encodeURIComponent(b)}),and(requester_id.eq.${encodeURIComponent(b)},addressee_id.eq.${encodeURIComponent(a)}))&status=in.(pending,accepted)&select=id&limit=1`, undefined);
...
router.get("/conversations", async (req, res): Promise<void> => {
  ...
  try {
    const m = await sb(`/rest/v1/direct_conversation_members?user_id=eq.${encodeURIComponent(a.user.id)}&select=conversation_id`, undefined);
    ...
    const r = await sb(`/rest/v1/direct_conversations?id=in.(${ids.join(",")})&select=*`, undefined);
...
router.post("/conversations", async (req, res): Promise<void> => {
  ...
  if (!(await hasRelationship(a.user.id, ids[0]))) {
    res.status(403).json({ error: "Add this person as a friend first." }); return;
  }
  try {
    const existing = await sb(`/rest/v1/direct_conversation_members?user_id=eq.${encodeURIComponent(a.user.id)}&select=conversation_id`, undefined);
    ...
    for (const x of memberships) {
      const mr = await sb(`/rest/v1/direct_conversation_members?conversation_id=eq.${x.conversation_id}&select=user_id`, undefined);
      const members = await readJson<Array<{ user_id: string }>>(mr) ?? [];
      if (members.map(v => v.user_id).sort().join(",") === target) {
        const cr = await sb(`/rest/v1/direct_conversations?id=eq.${x.conversation_id}&select=*`, undefined);
        const d = await readJson<unknown>(cr);
        if (Array.isArray(d) && d[0]) { res.json(d[0]); return; }
      }
    }
    const c = await sb("/rest/v1/direct_conversations?select=*", undefined, { method: "POST", body: "{}", headers: { Prefer: "return=representation" } });
    ...
    const mr = await sb("/rest/v1/direct_conversation_members", undefined, {
      method: "POST",
...
router.get("/conversations/:id/messages", async (req, res): Promise<void> => {
  const a = await auth(req, res); if (!a) return;
  try {
    const m = await sb(`/rest/v1/direct_conversation_members?conversation_id=eq.${encodeURIComponent(req.params.id)}&user_id=eq.${encodeURIComponent(a.user.id)}&select=user_id&limit=1`, undefined);
    if (!(await readJson<unknown[]>(m) ?? []).length) { res.status(403).json({ error: "You are not a member of this chat." }); return; }
    const r = await sb(`/rest/v1/chat_messages?conversation_id=eq.${encodeURIComponent(req.params.id)}&select=*&order=created_at.asc&limit=100`, undefined);
    res.status(r.ok ? 200 : 502).json(await readJson<unknown>(r) ?? []);
...
router.post("/conversations/:id/messages", async (req, res): Promise<void> => {
  ...
  try {
    const m = await sb(`/rest/v1/direct_conversation_members?conversation_id=eq.${encodeURIComponent(req.params.id)}&user_id=eq.${encodeURIComponent(a.user.id)}&select=user_id&limit=1`, undefined);
    if (!(await readJson<unknown[]>(m) ?? []).length) { res.status(403).json({ error: "You are not a member of this chat." }); return; }
    const r = await sb("/rest/v1/chat_messages?select=*", undefined, {
      method: "POST",
