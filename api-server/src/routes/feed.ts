...
async function interactionCounts(postId:number, token:string, userId:string){
 const [likes,comments,shares]=await Promise.all([
  supabase(`/rest/v1/post_likes?post_id=eq.${encodeURIComponent(String(postId))}&select=user_id`,token,{method:"GET"}),
  supabase(`/rest/v1/post_comments?post_id=eq.${encodeURIComponent(String(postId))}&select=id`,token,{method:"GET"}),
  supabase(`/rest/v1/post_shares?post_id=eq.${encodeURIComponent(String(postId))}&select=user_id`,token,{method:"GET"})
 ]);
