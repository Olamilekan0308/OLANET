...
type Post = { id: number; user_id: string; content?: string | null; created_at?: string | null; circle_id?: number | null; image_url?: string | null; media_url?: string | null; };
...
router.get("/feed", async (req, res): Promise<void> => {
  ...
  const profilesResponse = await supabase(`/rest/v1/profiles?id=in.(${userIds.map(encodeURIComponent).join(",")})&select=id,full_name,username,avatar_url,bio`, a.token, { method: "GET" });
  const profiles = await readJson<Profile[]>(profilesResponse) ?? [];
  const byUser = new Map(profiles.map(p => [p.id, p]));

  const enriched = await Promise.all(posts.map(async post => {
    const interactionResponse = await supabase(`/posts/${encodeURIComponent(String(post.id))}/interactions`, a.token, { method: "GET" });
    const interactions = await readJson<{ likes?: number; liked?: boolean; comments?: unknown[]; shares?: number }>(interactionResponse) ?? {};
    return {
      ...post,
      author: byUser.get(post.user_id) ?? { id: post.user_id, full_name: "OLANET member" },
      ...
      shares: Number(interactions.shares || 0),
    };
  }));
...
router.post("/feed", async (req, res): Promise<void> => {
  ...
  const data = await readJson<unknown>(created);
  if (!created.ok) { res.status(created.status).json(data ?? { error: "Could not create post." }); return; }
