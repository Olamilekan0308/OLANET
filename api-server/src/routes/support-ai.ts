...
router.post("/support/ai", async (req, res): Promise<void> => {
  try {
    ...
    if (!session) return;
    const body = req.body as { message?: string; sessionId?: number };
    ...
    if (!apiKey) { res.status(503).json({ error: "OLANET support is not configured yet." }); return; }
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model: OPENAI_MODEL, input: [{ role: "developer", content: system }, { role: "user", content: message }], max_output_tokens: 1200 }),
    });
    const data = await response.json().catch(() => ({})) as { output_text?: string; error?: { message?: string } };
    if (!response.ok) { res.status(502).json({ error: data.error?.message || "OLANET support is temporarily unavailable." }); return; }
    const answer = data.output_text?.trim() || "I couldn't produce an answer right now. Please try again.";

    let sessionId = Number.isInteger(body.sessionId) ? body.sessionId! : null;
    if (!sessionId) {
      const created = await supabase("/rest/v1/ai_sessions", session.accessToken, { method: "POST", body: JSON.stringify({ user_id: session.user.id, session_type: "support", title: "OLANET AI Support" }) });
      const rows = await readJson<Array<{ id: number }>>(created);
    ...
    req.log.error({ error }, "OLANET support AI request failed");
    res.status(502).json({ error: error instanceof Error ? error.message : "OLANET support is temporarily unavailable." });
  }
});
