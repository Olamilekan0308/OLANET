...
async function getSession(req, res) {
  ...
  if (c[ACCESS_COOKIE]) {
    const response = await sb("/auth/v1/user", {
      headers: { Authorization: `Bearer ${c[ACCESS_COOKIE]}` },
    });
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        error: "Could not load OLANET users.",
        details: data,
      });
    }
    ...
    const people = (Array.isArray(data) ? data : [])
      ...
      .filter(person => {
        ...
        return normalize(person.full_name).includes(needle) || normalize(person.username).includes(needle);
      });
