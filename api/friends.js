...
export function registerFriendRoutes(app) {
  app.get("/api/friends", async (req, res) => {
    ...
    try {
      const r = await supabase(`/rest/v1/friendships?or=(requester_id.eq.${encodeURIComponent(a.user.id)},addressee_id.eq.${encodeURIComponent(a.user.id)})&select=*`, { headers: { Authorization: `Bearer ${a.token}` } });
  ...
  app.get("/api/friends/requests", async (req, res) => {
    const a = await auth(req, res); if (!a) return;
    try {
      const r = await supabase(`/rest/v1/friendships?addressee_id=eq.${encodeURIComponent(a.user.id)}&status=eq.pending&select=*`, { headers: { Authorization: `Bearer ${a.token}` } });
      const data = await json(r);
      return res.status(r.ok ? 200 : r.status).json(Array.isArray(data) ? data : []);
  ...
  app.post("/api/friends/:userId", async (req, res) => {
    ...
    try {
      ...
      const r = await supabase("/rest/v1/friendships?select=*", {
        method: "POST",
        headers: { Authorization: `Bearer ${a.token}`, Prefer: "return=representation" },
        body: JSON.stringify({ requester_id: a.user.id, addressee_id: target, status: "pending" }),
      });
      const data = await json(r);
      if (!r.ok) return res.status(r.status).json(data || { error: "Could not send friend request." });
  ...
  app.patch("/api/friends/:friendshipId", async (req, res) => {
    ...
    try {
      const r = await supabase(`/rest/v1/friendships?id=eq.${encodeURIComponent(req.params.friendshipId)}&addressee_id=eq.${encodeURIComponent(a.user.id)}&status=eq.pending`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${a.token}`, Prefer: "return=representation" },
