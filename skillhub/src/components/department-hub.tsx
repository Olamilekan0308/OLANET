...
import { ArrowLeft, ArrowRight, Calculator, Check, ChevronRight, Send, Users, Zap, type LucideIcon } from 'lucide-react';
...
type DepartmentPost = { id: number | string; user_id?: string; content?: string | null; body?: string | null; created_at?: string | null };
...
export default function DepartmentHub() {
  ...
  async function loadDetail(id: number) {
    try {
      setError('');
      const [detailR, calcR, membersR, postsR] = await Promise.all([
        fetch(`/api/circles/${id}`, { credentials:'include' }),
        fetch(`/api/circles/${id}/calculators`, { credentials:'include' }),
        fetch(`/api/circles/${id}/members`, { credentials:'include' }),
        fetch(`/api/circles/${id}/posts`, { credentials:'include' }),
      ]);
      const detail = await detailR.json();
      const calc = await calcR.json();
      const mem = await membersR.json();
      const postData = await postsR.json().catch(() => []);
      if (!detailR.ok) throw new Error(detail?.error || 'Department not found');
      setSelected(detail); setCalculators(calc.calculators ?? []); setMembers(mem.members ?? []); setPosts(postsR.ok && Array.isArray(postData) ? postData : []);
  ...
  async function toggleJoin(circle: Circle) {
    ...
    const joined = !circle.isMember;
    setSelected((s) => s ? {...s, isMember: joined, memberCount: Math.max(0, s.memberCount + (joined ? 1 : -1))} : s);
    setCircles((xs) => xs.map(x => x.id === circle.id ? {...x, isMember: joined, memberCount: Math.max(0, x.memberCount + (joined ? 1 : -1))} : x));
  }

  async function createPost() {
    if (!selected || !selected.isMember || !postText.trim() || posting) return;
    try {
      setPosting(true); setError('');
      const r = await fetch(`/api/circles/${selected.id}/posts`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({content:postText.trim()}) });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(d?.error || 'Unable to publish post');
  ...
  if (detailId) return <div className="min-h-[100dvh] bg-[#f8f2e8] p-4 text-[#1d4348] sm:p-8"><div className="mx-auto max-w-5xl"><a href="/circles" className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-[#527075]"><ArrowLeft size={16}/> All departments</a>{error && <Notice text={error}/>} {!selected ? <Loading/> : <><div className="overflow-hidden rounded-[24px] border border-[#dfd2c0] bg-[#fffaf1]"><div className="p-7" style={{background:`linear-gradient(135deg, ${colors[selected.id % colors.length]}, #fffaf1)`}}><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fffaf1]/80"><Zap size={27}/></div><h1 className="mt-5 text-3xl font-bold">{selected.name}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#527075]">{selected.description || 'A practical OLANET department for learning, collaboration and real-world work.'}</p></div><div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-5"><Stat icon={Users} value={selected.memberCount.toLocaleString()} label="members"/><Stat icon={Calculator} value={String(calculators.length)} label="calculators"/></div><button onClick={() => void toggleJoin(selected)} className="rounded-xl bg-[#2f817d] px-5 py-3 text-sm font-bold text-white">{selected.isMember ? 'Joined' : 'Join department'}</button></div></div><div className="mt-5 grid gap-5 lg:grid-cols-[1fr_300px]">
<section className="space-y-5">
  <div className="rounded-2xl border border-[#dfd2c0] bg-[#fffaf1] p-5"><h2 className="text-xl font-bold">Department feed</h2><p className="mt-1 text-sm text-[#789093]">Posts from people in {selected.name}.</p>{selected.isMember ? <div className="mt-4 rounded-xl border border-[#eadfce] bg-[#f8f2e8] p-3"><textarea value={postText} onChange={e => setPostText(e.target.value)} placeholder={`Share something with ${selected.name}…`} rows={3} maxLength={2000} className="w-full resize-none bg-transparent p-1 text-sm outline-none"/><div className="mt-2 flex items-center justify-between"><span className="text-[10px] text-[#789093]">{postText.length}/2000</span><button disabled={!postText.trim() || posting} onClick={() => void createPost()} className="inline-flex items-center gap-2 rounded-lg bg-[#2f817d] px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Send size={14}/>{posting ? 'Posting…' : 'Post'}</button></div></div> : <div className="mt-4 rounded-xl bg-[#f8f2e8] p-4 text-sm text-[#789093]">Join this department to publish posts and participate in its feed.</div>}</div>
  <div className="space-y-3">{posts.map(post => <article key={String(post.id)} className="rounded-2xl border border-[#dfd2c0] bg-[#fffaf1] p-5"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#dceceb] text-xs font-bold text-[#2f817d]">OL</div><div><p className="text-sm font-bold">Department member</p><p className="text-[10px] text-[#789093]">{formatDate(post.created_at)}</p></div></div><p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-[#36575b]">{post.content ?? post.body ?? ''}</p></article>)}{!posts.length && <div className="rounded-2xl border border-dashed border-[#cbbda9] bg-[#fffaf1] p-10 text-center text-sm text-[#789093]">No department posts yet. Be the first to start the conversation.</div>}</div>
  <section className="rounded-2xl border border-[#dfd2c0] bg-[#fffaf1] p-5"><h2 className="text-xl font-bold">Department calculators</h2><p className="mt-1 text-sm text-[#789093]">Tools specific to {selected.name}. No giant junk drawer of unrelated calculators.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{calculators.map(c => <div key={c.id} className="rounded-xl border border-[#eadfce] bg-[#f8f2e8] p-4"><div className="flex items-start gap-3"><div className="rounded-xl bg-[#e5f0e9] p-2 text-[#2f817d]"><Calculator size={17}/></div><div><h3 className="font-bold">{c.name}</h3><p className="mt-1 text-xs leading-5 text-[#789093]">{c.description}</p></div></div></div>)}</div></section>
</section>
<aside className="rounded-2xl border border-[#dfd2c0] bg-[#fffaf1] p-5"><h2 className="text-xl font-bold">Members</h2><div className="mt-4 space-y-3">{members.slice(0,12).map(m => <div key={m.id} className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-[#dceceb] text-xs font-bold text-[#2f817d]">{m.avatar_url ? <img src={m.avatar_url} alt="" className="h-full w-full object-cover"/> : (m.full_name || m.username || '?').slice(0,2).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-bold">{m.full_name || m.username || 'OLANET member'}</p><p className="text-[10px] text-[#789093]">{m.role || 'member'}</p></div></div>)}{!members.length && <p className="text-sm text-[#789093]">No members to display yet.</p>}</div></aside>
</div></>}</div></div>;
...
function formatDate(value?: string | null){ if(!value) return 'Just now'; const d=new Date(value); if(Number.isNaN(d.getTime())) return 'Just now'; return d.toLocaleString([], {dateStyle:'medium',timeStyle:'short'}); }
