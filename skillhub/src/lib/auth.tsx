...
type AuthContextValue = {
  ...
  logout: () => Promise<void>;
  updateProfile: (updates: ProfileUpdateInput) => Promise<NonNullable<AuthUser['profile']>>;
  uploadAvatar: (file: File) => Promise<NonNullable<AuthUser['profile']>>;
};
...
async function profileRequest(path: string, init?: RequestInit) {
  ...
  if (!data.profile) throw new Error('Supabase returned no profile after saving.');
  return data.profile;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async signup(fullName, email, password) {
      ...
      if (data.user && !data.needsEmailConfirmation && !data.profileError) {
        setUser(data.user);
        setStatus('authenticated');
      }
      return data;
    ...
    async updateProfile(updates) {
      const profile = await profileRequest('me', {
        method: 'PUT',
    ...
    async uploadAvatar(file) {
      const profile = await profileRequest('avatar', {
        method: 'POST',
